...
void tcpServerTask(void * pvParameters)
{
	...
	xTaskCreate(tcpServerRecvTask,"tcpRecvTask",4096,NULL,4, &tcpServerRecvTaskHandle);		 //Start receive task
	tcpServerSemaphore = xSemaphoreCreateMutex();
	
	for(;;)
	{
		if(xQueueReceive(senxorFrameQueue, &mSenxorFrameObj, portMAX_DELAY) && isClientConnected)
		{
